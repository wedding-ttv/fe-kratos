...
export function handleGetFlowError<S>(
  ...
  return async (err: AxiosError) => {
    // Type assertion since we know the shape of the error response
    const errorData = (err.response?.data as { error?: { id: string } })?.error
    switch (errorData?.id) {
      case "session_inactive":
        await router.push("/login?return_to=" + window.location.href)
        return
      case "session_aal2_required":
        if (err.response?.data && typeof err.response.data === 'object' && 'redirect_browser_to' in err.response.data) {
          const redirectTo = new URL(err.response.data.redirect_browser_to as string)
      ...
      case "session_refresh_required":
        ...
        if (err.response?.data && typeof err.response.data === 'object' && 'redirect_browser_to' in err.response.data) {
          window.location.href = err.response.data.redirect_browser_to as string
        }
        return
